...
class UCF101VideoDataset(Dataset):
    ...
    def __init__(
        ...
        # Replace the sorted() line in your dataset __init__ with this:
        raw_labels = [self.id_to_label[i] for i in range(len(self.id_to_label))]
        
        # Convert CamelCase "ApplyEyeMakeup" -> "apply eye makeup" for natural text matching
        import re
        self.unique_labels = [
        ...
        test_path = os.path.join(base_dir, self.video_list[0][0].replace('/', os.sep))
        print(f"Checking if first video exists at: {test_path}")
